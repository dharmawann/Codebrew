...
export class Game {
  constructor(canvas, aiMode) {
    ...
    this._onKeyDown = (e) => {
      ...
      if (this.minigame.active) {
        ...
        if (['w', 'a', 's', 'd', 'e', 't', 'r', 'f', ' ', 'shift', 'escape'].includes(k)) {
          e.preventDefault();
        }
        return;
      }
      ...
      if (!this.keys[k]) {
        ...
        if (k === 't') {
          this.startMinigame('cool', this.temp);
          if (this.aiMode) {
            this.ai.push('MANUAL OVERRIDE — COOLING VENTS', '#ff8822');
          }
        }

        if (k === 'r') {
          this.startMinigame('shield', this.hull);
          if (this.aiMode) {
            this.ai.push('MANUAL OVERRIDE — SHIELD MATRIX', '#4499ff');
          }
        }

        if (k === 'f') {
          this.startMinigame('fog', this.humidity);
          if (this.aiMode) {
            this.ai.push('MANUAL OVERRIDE — DEHUMIDIFIER', '#00ffcc');
          }
        }
      }

      if (['w', 'a', 's', 'd', 'e', 't', 'r', 'f', ' ', 'shift'].includes(k)) {
        e.preventDefault();
      }
    };
    ...
    this._onMouseMove = (e) => {
      if (!this.minigame.active || this.minigame.type !== 'fog') return;

      const rect = this.canvas.getBoundingClientRect();
      const mouseX = (e.clientX - rect.left) * (this.canvas.width / rect.width);
      const mouseY = (e.clientY - rect.top) * (this.canvas.height / rect.height);

      const idx = this.minigame.getFogLineAtMouse(
        mouseX,
        mouseY,
        this.W(),
        this.H()
      ...
      if (idx !== -1) {
        this.minigame.wipeLineAtIndex(idx);
      }
    };
  ...
  startMinigame(type, param) {
    ...
    if (this.aiMode) {
      if (type === 'cool') {
        this.ai.push('MINIGAME: COOLING VENTS ONLINE', '#ff8822');
      } else if (type === 'shield') {
        this.ai.push('MINIGAME: SHIELD MATRIX ONLINE', '#4499ff');
      } else if (type === 'fog') {
        this.ai.push('MINIGAME: DEHUMIDIFIER ONLINE', '#00ffcc');
      }
    }
  }
  ...
  handleMinigameResult() {
    ...
    if (!this.minigame.result || this.minigameRewardApplied) {
      return;
    }
    ...
    if (this.minigame.result === 'success') {
      if (this.minigame.type === 'cool') {
        ...
        this.showEffectText('COOLING STABILISED', '#00ff99');

        if (this.aiMode) {
          this.ai.push('COOLING SUCCESS — THERMAL LOAD REDUCED', '#00ff99');
        }
      }

      if (this.minigame.type === 'shield') {
        this.shieldActive = 320;
        this.showEffectText('SHIELD MATRIX ONLINE', '#4499ff');

        if (this.aiMode) {
          this.ai.push('SHIELD SUCCESS — IMPACT BUFFER ACTIVE', '#4499ff');
        }
      }

      if (this.minigame.type === 'fog') {
        this.humidity = Math.max(28, this.humidity - 30);
        this.fogAlpha = clamp((this.humidity - 63) / 37, 0, 0.75);
        this.showEffectText('HUMIDITY PURGED', '#00ffcc');

        if (this.aiMode) {
          this.ai.push('DEHUMIDIFIER SUCCESS — FOG LEVEL DROPPING', '#00ffcc');
        }
      }
      ...
      if (this.minigame.type === 'shield') {
        this.showEffectText('SHIELD FAILED', '#ff4422');
      }

      if (this.minigame.type === 'fog') {
        this.humidity = Math.min(100, this.humidity + 4);
        this.fogAlpha = clamp((this.humidity - 63) / 37, 0, 0.75);
        this.showEffectText('DEHUMIDIFIER FAILED', '#ff4422');
      }

      if (this.aiMode) {
        this.ai.push('MINIGAME FAILED — SYSTEM STILL UNSTABLE', '#ff4422');
      }
    }
  }
