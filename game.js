...
export class Game {
  ...
  animateSplash(sc) {
    ...
    const frame = () => {
      ...
      for (const s of splashStars) {
        ...
        sctx.beginPath(); sctx.arc(s.x * sw, s.y * sh, s.s, 0, 6.28); sctx.fill();
      }
      sctx.globalAlpha = 1;
  ...
  _handleCollision(a, p) {
    if (this.shieldActive > 0) {
      ...
      a.z = rnd(1500, 2200); a.x = rnd(-1700, 1700);
    } else {
      ...
      a.z = rnd(1600, 2400); a.x = rnd(-1700, 1700);
      if (this.aiMode)
  ...
  render() {
    ...
    // Crosshair + HUD text
    this.hud.drawHUD(
      this.hull, this.speed, this.shieldActive,
      this.aiMode, this.aiCooldown, this.frame
    );
