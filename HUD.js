export class HUD {
  ...
  drawCockpit() {
    ...
    ctx.strokeStyle = 'rgba(0,185,225,0.42)'; ctx.lineWidth = 2;
    ctx.strokeRect(w * 0.04, h * 0.02, w * 0.92, vH - h * 0.025);

    for (let i = 1; i < 4; i++) {
      ctx.strokeStyle = 'rgba(0,180,220,0.11)'; ctx.lineWidth = 5;
      ctx.beginPath();
      const px = w * 0.04 + w * 0.92 * i / 4;
      ctx.moveTo(px, h * 0.02); ctx.lineTo(px, vH); ctx.stroke();
    }
    for (let i = 0; i < 8; i++) {
    ...
    ctx.fillStyle = 'rgba(1,6,16,.97)'; ctx.fillRect(0, vH, w, h - vH);
    ctx.strokeStyle = 'rgba(0,200,255,0.13)'; ctx.lineWidth = 0.5;
    for (let i = 0; i < 15; i++) {
      const x = w * i / 15;
  ...
  drawHUD(hull, speed, shieldActive, aiMode, aiCooldown, frame) {
    ...
    [[cx-30,cy,cx-9,cy],[cx+9,cy,cx+30,cy],[cx,cy-30,cx,cy-9],[cx,cy+9,cx,cy+30]]
      .forEach(([x1,y1,x2,y2]) => {
    ...
    if (shieldActive > 0) {
      ...
      ctx.strokeStyle = `rgba(0,130,255,${pulse * 0.65})`; ctx.lineWidth = 2;
      ctx.beginPath();
  ...
  drawMFDs(game) {
    ...
    const dY = h * 0.618, dH = h * 0.368;
    const gap = w * 0.011, dW = (w - gap * 4) / 3;
    const panels = [
      { x: gap,           type: 'eng'    },
    ...
    for (const p of panels) {
      ...
      const bc = crit ? (game.frame % 8 < 4 ? '#ff4400' : '#330000') : 'rgba(0,185,205,.52)';

      ctx.fillStyle = 'rgba(0,5,13,.94)'; ctx.fillRect(p.x, dY, dW, dH);
      ctx.strokeStyle = bc; ctx.lineWidth = crit ? 2 : 1;
  ...
  _drawEng(x, y, w, h, fs, game) {
    const ctx = this.ctx;
    const fog = game.fogAlpha, tc = game.temp > 86;
    if (tc && game.frame % 6 < 3) {
      ctx.fillStyle = 'rgba(255,15,0,.065)'; ctx.fillRect(x, y, w, h);
    }

    const pw = w - 18, ph = Math.round(h * 0.21);
    ...
    for (let i = 0; i < 2; i++) {
      ...
      ctx.textAlign = 'right'; ctx.fillText(disp[i], x + w - 7, ly + fs); ctx.textAlign = 'left';

      ctx.fillStyle = 'rgba(0,28,48,.85)'; ctx.fillRect(x + 9, ly + fs + 5, pw, ph);
      const arr = hists[i], maxV = i === 0 ? 135 : 100;
      ctx.beginPath();
      arr.forEach((v, j) => {
      ...
      ctx.strokeStyle = cols[i]; ctx.lineWidth = 1.5; ctx.stroke();
    }
    ...
    if (tc) {
      ...
      ctx.textAlign = 'center'; ctx.fillText('TEMP CRITICAL', x + w / 2, warningY); ctx.textAlign = 'left';
    } else if (game.humidity > 72) {
      ctx.fillStyle = '#ffaa00';
      ctx.font = `${Math.round(fs * 0.85)}px 'Courier New'`;
      ctx.textAlign = 'center'; ctx.fillText('HUMIDITY ALERT', x + w / 2, warningY); ctx.textAlign = 'left';
    }
    ...
    if (fog > 0.06) {
      ...
      ctx.font = `bold ${Math.round(fs * 0.9)}px 'Courier New'`;
      ctx.textAlign = 'center'; ctx.fillText('CONDENSATION', x + w / 2, y + h / 2); ctx.textAlign = 'left';
    }
  }
  ...
  _drawNeural(x, y, w, h, fs, game) {
    const ctx = this.ctx;
    if (!game.aiMode) {
      ctx.fillStyle = 'rgba(255,30,0,.03)'; ctx.fillRect(x, y, w, h);
      for (let i = 0; i < 8; i++) {
        if (Math.random() < 0.45) {
          const gy = y + Math.random() * h, gw = 20 + Math.random() * (w * 0.72);
    ...
    const lH = fs * 1.72; let cy = y + lH * 0.6;
    for (const e of game.ai.history) {
      if (cy > y + h - lH * 2) break;
      const alpha = Math.min(1,
        e.age < 22 ? e.age / 22 :
        e.age > 210 ? Math.max(0, 1 - (e.age - 210) / 75) : 1
      );
      ctx.globalAlpha = alpha; ctx.fillStyle = e.col;
      ctx.font = `${Math.round(fs * 0.88)}px 'Courier New'`;
      const words = e.msg.split(' '); let line = '';
      for (const word of words) {
        const test = line + (line ? ' ' : '') + word;
        if (ctx.measureText(test).width > w - 18 && line) {
          ctx.fillText(line, x + 9, cy); cy += lH; line = word;
        } else line = test;
      }
      if (line) { ctx.fillText(line, x + 9, cy); cy += lH; }
      cy += 5;
    }
    ctx.globalAlpha = 1;
    ...
    if (game.aiCooldown > 0) {
      ctx.fillStyle = 'rgba(0,28,48,.85)'; ctx.fillRect(x + 9, y + h - 22, bw, 10);
  ...
  _drawNav(x, y, w, h, fs, game) {
    ...
    const items = [
      ...
      { l: 'TEMPERATURE',   v: `${game.temp.toFixed(1)}°C`,        c: game.temp>86?'#ff4400':game.temp>66?'#ff8800':'#00ccff' },
      { l: 'HUMIDITY',      v: `${game.humidity.toFixed(0)}%`,     c: game.humidity>72?'#ff8800':'#00ffcc' },
    ];
    const lH = h * 0.145;
    items.forEach((item, i) => {
      const iy = y + 8 + i * lH;
      ctx.fillStyle = '#334'; ctx.font = `${Math.round(fs*0.8)}px 'Courier New'`;
      ctx.fillText(item.l, x + 8, iy + fs * 0.9);
      ctx.fillStyle = item.c; ctx.font = `bold ${Math.round(fs*1.06)}px 'Courier New'`;
      ctx.fillText(item.v, x + 8, iy + fs * 1.95);
    });

    const bW = w - 16, bY = y + h * 0.66;
    ctx.fillStyle = '#223'; ctx.font = `${Math.round(fs*0.8)}px 'Courier New'`;
    ctx.fillText('HULL STATUS', x + 8, bY);
    ctx.fillStyle = 'rgba(0,28,48,.85)'; ctx.fillRect(x + 8, bY + 5, bW, 9);
    ...
    ctx.fillStyle = '#00ffcc'; ctx.font = `bold ${Math.round(fs*1.65)}px 'Courier New'`;
  ...
  drawFog(fogAlpha, rnd) {
    ...
    const ctx = this.ctx;
    const w = this.W(), h = this.H(), vH = h * 0.60;
    ctx.fillStyle = `rgba(160,215,195,${fogAlpha * 0.32})`; ctx.fillRect(0, 0, w, vH);
  ...
  drawFlicker(flickerAlpha, rnd) {
    ...
    const ctx = this.ctx;
    ctx.fillStyle = `rgba(0,205,255,${flickerAlpha * 0.055})`; ctx.fillRect(0, 0, this.W(), this.H());
